...
<?php
	...
	$query="SELECT Users.username AS username, Users.username AS name, rating, comment, links.id as link, descrip, clicks FROM ratings, comments, Users, links WHERE ratings.band='$band' AND comments.band='$band' AND links.band='$band' AND ratings.user=comments.user AND ratings.user=links.user AND ratings.user=Users.id AND Users.username='".$_SESSION['user']."'";

	$query="SELECT Users.username AS username, Users.username AS name, rating, comment, descrip, clicks, links.id as link FROM Users LEFT JOIN ratings ON Users.id=ratings.user AND ratings.band='$band' LEFT JOIN comments ON Users.id=comments.user AND comments.band='$band' LEFT JOIN links ON Users.id=links.user AND links.band='$band' GROUP BY Users.id WHERE Users.username='".$_SESSION['user']."'";
