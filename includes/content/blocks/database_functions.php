...
function getBPname($bandPlevel)
{
    //This function checks $source table Users for the username of $userid
    global $master;
    $sql = "select `name` from `band_priorities` where `level`='$bandPlevel'";
    $res = mysql_query($sql, $master);
    $urow = mysql_fetch_array($res);
    $bname = $urow['name'];
    return $bname;
...
function getDtname($dateid)
{
    //This function checks $source table stages for the name of $stageid
    global $master;
    $sql = "select `name` from `dates` where `id`='$dateid' and `deleted`!='1'";
    $res = mysql_query($sql, $master);


    $srow = mysql_fetch_array($res);
    $sname = $srow['name'];
//    error_log("dateName: " . $sname);
...
function getDayAndStageOfSetsByBandInFest($bandid)
{
    //This function returns an array containing keyed to the id of each each set by the band, containing the stage and day
    global $master, $fest;
    $sql = "select `id`, `day`, `stage`, `date` from `sets` where `deleted` != '1' and `festival`='$fest' AND `band`='$bandid'";
    $res = mysql_query($sql, $master);
...
function getSetDetailsForBand($bandid)
{
    //This function returns an array containing the id of each band in the festival
    global $master, $fest;
    $sql = "select `id`, `festival`, `festival_series`, `band`, `day`, `stage`, `start`, `end`, `date` from `sets` where `deleted` != '1' and `festival`='$fest' AND `band`='$bandid'";
    $res = mysql_query($sql, $master);
    $num = mysql_num_rows($res);
    while ($row = mysql_fetch_array($res)) {
        $sets[] = $row;
...
{
    global $master, $fest;
    $sql = "select max(`id`) as id from `dates` where `deleted`!='1' and `festival`='$fest'";
    $res = mysql_query($sql, $master);
    if (mysql_num_rows($res) > 0) {
        $row = mysql_fetch_array($res);
        $result = $row['id'];
...
function displayBandPicArray($bands)
{
    global $displayedBandID, $bandsDisplayed, $bandsToDisplay;
//    echo "Bands displayed: ". $bandsDisplayed ."//" . $bandsToDisplay;
    shuffle($bands);
    $picArray = array(
        array(0, 0),
        array(0, 0),
...
//		echo $i." is in for loop<br>";
        foreach ($bands as &$big) {
            if ($bandsDisplayed >= $bandsToDisplay) return;
            //               echo "Band ID: " . $big. "<br />";
            switch ($x) {
                case 0:
                    $shapeCode = 15;
...
//				echo count($displayedBandID)." is count of displayed bands.";
    ...
    } while ($bandDisplayedThisLoop == 1 && $bandsToDisplay > $bandsDisplayed);
    if ($bandsToDisplay > $bandsDisplayed) {
        foreach ($bands as $leftOver) {
...
function getGenresForAllBandsInFest($user)
{
    ...
    if (mysql_num_rows($res) > 0) {
        while ($row = mysql_fetch_array($res)) {
            $genre = getBandGenreID($row['id'], $user);
            if (!in_array($genre, $result)) $result[] = $genre;
        }
        return $result;
    } else return false;
}

function getAllBandsOfAGenreInFest($user, $genre)
{
    //Find genre of every band in main
    global $master, $fest;
    $sql = "select `band` as id from `band_list` where `festival`='$fest' and `deleted`!='1' order by rand()";
    ...
    if (mysql_num_rows($res) > 0) {
        while ($row = mysql_fetch_array($res)) {
            $bGenre = getBandGenreID($row['id'], $user);
            if ($bGenre == $genre) $result[] = $row['id'];
        }
        return $result;
    } else return false;
}
...
function getBandSetsByFestival($band)
{
    ...
    global $master, $fest;
    $sql = "SELECT `id`, `day`, `stage`, `start`, `end`, `date` FROM `sets` WHERE `band`='$band' AND `festival`='$fest' AND `deleted`!='1'";
//    error_log(print_r($sql, TRUE));
    $res = mysql_query($sql, $master);
    if (mysql_num_rows($res) > 0) {
        while ($row = mysql_fetch_array($res)) {
