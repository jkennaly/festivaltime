...
If(mysql_num_rows($res)>0) {
	...
	If(mysql_num_rows($res1)>0) {
		...
		while($row1 = mysql_fetch_array($res1)) {
			...
			$discuss .= "<input type=\"submit\" value=\"Send response\" class=\"reply".$row['id']."\"><input type=\"hidden\" name=\"comment\" value=\"".$row['id']."\"><input type=\"hidden\" name=\"discuss_table\" value=\"$discuss_table\"></form><a id=\"displayText$user\" title=\"Click to toggle discussion\" href=\"#\" onclick=\"toggle('toggleText".$user."', 'displayText".$user."', '$user', '".$row['id']."', 'reply".$row['id']."');return false;\">Hide Discussion</a></div>";
	} else {
...
If(mysql_num_rows($res)>0) {
	...
	If(mysql_num_rows($resultcurrent) == 0){
		...
		If(mysql_num_rows($res1)>0) {
			...
			$discuss .= "<br /><form action=\"$post_target\" method=\"post\"><textarea rows=\"16\" cols=\"64\" name=\"new_reply\"></textarea>";
			$discuss .= "<input type=\"submit\" value=\"Send response\" class=\"reply".$row['id']."\"><input type=\"hidden\" name=\"comment\" value=\"".$row['id']."\"><input type=\"hidden\" name=\"discuss_table\" value=\"$discuss_table\"></form><a id=\"displayText".$row['user']."\" title=\"Click to toggle discussion\" href=\"#\" onclick=\"toggle('toggleText".$row['user']."', 'displayText".$row['user']."', '$user', '".$row['id']."', 'reply".$row['id']."');return false;\">Hide Discussion</a></div>";
		} else  {
		    //Check to see if the current user is current on the comment
		    If($stat == "New discussion!") {      
                $query = "UPDATE comments SET discuss_current=CONCAT(discuss_current,'--$user--') where id='".$row['id']."'";
