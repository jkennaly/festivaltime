...
if (!empty($_POST['submitBandSchedule'])) {
    ...
    foreach ($_POST['newDay'] as $sID => $newDay) {
        if ($newDay > 0 && $_POST['newStage'][$sID] > 0) {
            $table = "sets";
            $cols = array("day", "stage");
            $vals = array($newDay, $_POST['newStage'][$sID]);
            $where = "`id`='" . $sID . "' AND `deleted`!='1'";
            updateRow($table, $cols, $vals, $where);
        }
    }
    foreach ($_POST['delete'] as $sID => $deletedSet) {
        if ($deletedSet == 1) deleteSet($sID);
    }
...
<form method="post" enctype="multipart/form-data">
    <div id="bandDisplay" class="bandSource">
        ...
        foreach ($bandList as $b) {
            ...
            <div id="band-<?php echo $b['band']; ?>" class="scheduleBand">
                <b><?php echo getBname($b['band']); ?></b><br/>
                Start Time:
                ...
                <input type="checkbox" name="delete[<?php echo $b['id']; ?>]" value="1"/>Delete this set<br/>
                Move set to:
                <select name="newDay[<?php echo $b['id']; ?>]">
                    <option value="0">Don't move</option>
                    <?php
                    foreach ($availDays as $a) {
                        if (!empty($_SESSION['setTimes']['day']) && $_SESSION['setTimes']['day'] == $a['id']) echo "<option selected=\"selected\" value=\"" . $a['id'] . "\">" . $a['name'] . "</option>";
                        else echo "<option value=\"" . $a['id'] . "\">" . $a['name'] . "</option>";
                ...
                <select name="newStage[<?php echo $b['id']; ?>]">
                    <option value="0">Don't move</option>
                    <?php
                    foreach ($availStages as $aS) {
                        if (!empty($_SESSION['setTimes']['stage']) && $_SESSION['setTimes']['stage'] == $aS['id']) echo "<option selected=\"selected\" value=\"" . $aS['id'] . "\">" . $aS['name'] . "</option>";
