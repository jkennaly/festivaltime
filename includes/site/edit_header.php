...
<div id="content">
	...
	Festival Series   
	<img alt="info_icon" src="includes/images/emblem-notice.png" title="The name of the festival series (e.g., Coachella, Bonnaroo, etc.)" />: 
	<select name="series">
	<?php 
	foreach($series as $s){
	...
	Start Time   
	<img alt="info_icon" src="includes/images/emblem-notice.png" title="Roughly the start time of the earliest band. If  you're not sure, earlier is better." />: 
	<select name="hour">
	<?php 
	...
	:00 
	<select name="period">
	<option <?php if($header['start_time'] < 3600*12 ) echo 'selected="selected"'; ?> value="AM">AM</option>
	<option <?php if($header['start_time'] > 3600*11 ) echo 'selected="selected"'; ?> value="PM">PM</option>
	</select><br />
	Day length  
	<img alt="info_icon" src="includes/images/emblem-notice.png" title="The number of hours in a festival day. 10:00 AM - 2:00 AM is 16. If  you're not sure, longer is better." />: 
	<select name="length">
	<?php 
	for($i = 1;$i < 25; $i++){
	...
	Cost 
	<img alt="info_icon" src="includes/images/emblem-notice.png" title="in FestivalTime credits; figure 1 per fest day" />: 
	<select name="cost">
	<?php 
	for($i = 0;$i < 6; $i++){
	...
	<img alt="info_icon" src="includes/images/emblem-notice.png" title="not days; Coachlla has two dates, Lollapalooza has 1" />: 
	<input type="number" name="num_dates" min="1" max="100" value="<?php echo $header['num_dates']; ?>"><br />
	Number of festival days 
	<img alt="info_icon" src="includes/images/emblem-notice.png" title="number of days in the festival-Coachella has 3, Bonnaroo has 4" />: 
